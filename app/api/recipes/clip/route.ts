  ...
  if (!html) {
    try {
      const wb = await fetch(
        `https://archive.org/wayback/available?url=${encodeURIComponent(body.url)}`,
        { signal: AbortSignal.timeout(6000) }
      );
      const wbData = await wb.json() as { archived_snapshots?: { closest?: { url?: string; available?: boolean } } };
      const archiveUrl = wbData?.archived_snapshots?.closest?.available
        ? wbData.archived_snapshots.closest.url
        : null;
